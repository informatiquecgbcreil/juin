...
def test_mode_simple_complet(admin_client):
    ...
    # Pill d'accès direct au planning (icône SVG + libellé).
    assert "Planning</a>" in page
    ...
    for lien in ("Locations", "Vie associative", "Transitions",
...
def test_mode_expert_range(admin_client):
    ...
    for lien in ("Planning de la semaine", "Salles &amp; lieux",
    ...
    assert "Vie associative (AG, CA)" in page
